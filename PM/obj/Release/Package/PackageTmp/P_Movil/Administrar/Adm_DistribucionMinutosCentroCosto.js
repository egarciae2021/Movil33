...
function CargarDatos() {
    $.ajax({
        url: "Adm_DistribucionMinutosCentroCosto.aspx/ListarCentroCosto", //PageMethod
        data: "{ 'inPagTam':'" + $("#tbCentroCosto").getGridParam("rowNum") + "'," + //Tamaño de pagina
                        "'inPagAct':'" + parseInt($("#tbCentroCosto").getGridParam("page")) + "'," + //Pagina actual
        ...
        success: function (result) {
            $("#tbCentroCosto")[0].addJSONData(result.d);
        },
...
function GuardarServiciosLinea(id, resp, idTabla) {
...
function CargarGrillaCentroCosto() {
    $("#tbCentroCosto").jqGrid({
        datatype: CargarDatos,
        colModel: colModelCentroCosto,
        shrinkToFit: false,
        height: AltoGrilla(),
        width: AnchoGrilla(),
        rownumbers: true,
        ...
        onSelectRow: function (id) {
            ...
            RegistroActual = $("#tbCentroCosto").jqGrid('getRowData', id);
            ...
            var editparameters = {
                ...
                "oneditfunc": null,
                "successfunc": null,
                "url": "Adm_DistribucionMinutosCentroCosto.aspx/GuardarServicios",
                "extraparam": {},
                "aftersavefunc": function (id, resp) { GuardarServiciosCentroCosto(id, resp); },
            ...
            if (id && id !== lastsel) {
                $("#tbCentroCosto").saveRow(lastsel, editparameters);
                $("#tbCentroCosto").editRow(id, true);
                lastsel = id;
            }
        },
        subGrid: true,
        ...
        subGridRowExpanded: function (subgrid_id, row_id) {
            ...
            $("#" + subgrid_id).html("<table id='" + subgrid_table_id + "' class='scroll tbSubGrid'></table><div id='" + pager_id + "' class='scroll'></div>");
    ...
    }).navGrid("#pagerCentroCosto", { edit: false, add: false, search: false, del: false });
    //$("#tbCentroCosto").jqGrid('setFrozenColumns');
}

function GuardarServiciosCentroCosto(id, resp) {
    var registro = $("#tbCentroCosto").jqGrid('getRowData', id);

    $.ajax({
        url: "Adm_DistribucionMinutosCentroCosto.aspx/GuardarServicios", //PageMethod
        data: "{'registro':'" + JSON.stringify(registro) + "'," +
                       "'vcValIli':'" + $("#hdfValorIlimitado").val() + "'," + //
                    ...
                    for (i in result.d.lstResultado) {
                        ...
                        while (RegistroAnterior["TipSer" + cont.toString()] != undefined) {
                            if (result.d.lstResultado[i]["TipSer"] == RegistroAnterior["TipSer" + cont.toString()] &&
                                ...
                                $("#tbCentroCosto").jqGrid('setCell', RegistroAnterior["P_vcCodCenCos"], "dcCan" + cont.toString(), RegistroAnterior["dcCan" + cont.toString()]);
                                break;
                            }
                            cont++;
                        }
...
function CargarCentroCosto() {
    $.ajax({
        url: "Adm_DistribucionMinutosCentroCosto.aspx/ListarColModelCentroCosto", //PageMethod
...
$(function () {
    ...
    function DimPosElementos() {
        Ancho = $(window).width();
        Alto = $(window).height();
        $("#tbCentroCosto").setGridWidth(AnchoGrilla());
        $("#tbCentroCosto").setGridHeight(AltoGrilla());
    }
