...
function CargarDatos() {
    $.ajax({
        url: "Adm_DistribucionMinutosNivel.aspx/ListarNivel", //PageMethod
        data: "{'inPagTam':'" + $("#tbNivel").getGridParam("rowNum") + "'," + //Tamaño de pagina
                        "'inPagAct':'" + parseInt($("#tbNivel").getGridParam("page")) + "'," + //Pagina actual
        ...
        success: function (result) {
            $("#tbNivel")[0].addJSONData(result.d);
        },
...
function CargarGrillaNivel() {

    $("#tbNivel").jqGrid({
        datatype: CargarDatos,
        colModel: colModelNivel,
        shrinkToFit: false,
        height: AltoGrilla(),
        width: AnchoGrilla(),
        rownumbers: true,
        ...
        onSelectRow: function (id) {
            ...
            RegistroActual = $("#tbNivel").jqGrid('getRowData', id);

            //var registro = JSON.stringify(RegistroActual);
            //alert(RegistroActual.dcCan1);
            //alert(RegistroActual(2));
            //alert(registro[2]);
            ...
            var editparameters = {
                ...
                "oneditfunc": null,
                "successfunc": null,
                "url": "Adm_DistribucionMinutosNivel.aspx/GuardarServicios",
                "extraparam": {},
                "aftersavefunc": function (id, resp) { GuardarServiciosNivel(id, resp); },
            ...
            if (id && id !== lastsel) {
                $("#tbNivel").saveRow(lastsel, editparameters);
                $("#tbNivel").editRow(id, true);
                lastsel = id;
            }
        },
        subGrid: true,
    ...
    }).navGrid("#pagerNivel", { edit: false, add: false, search: false, del: false });
    //$("#tbNivel").jqGrid('setFrozenColumns');
}

function GuardarServiciosNivel(id, resp) {
    var registro = $("#tbNivel").jqGrid('getRowData', id);
    var CanAct = RegistroAnterior.dcCan1;


    //alert(CanAct);
    //alert(registro.dcCan1);
    if (registro.dcCan1 == CanAct) {
        return;
    }

    $.ajax({
        url: "Adm_DistribucionMinutosNivel.aspx/GuardarServicios", //PageMethod
        data: "{'registro':'" + JSON.stringify(registro) + "'," +
                       "'vcValIli':'" + $("#hdfValorIlimitado").val() + "'," + //
        ...
        success: function (result) {
            ...
            if (result.d.ExisteLinea == "1") {
                if ($(result.d.lstResultado).length > 0) {
                    ...
                    contenido = "<h1>Ha sobrepasado el límite de crédito</h1>";
                    //contenido += "<ul>";
                    for (i in result.d.lstResultado) {
                        var cont = 1;
                        ...
                        while (RegistroAnterior["TipSer" + cont.toString()] != undefined) {
                            if (result.d.lstResultado[i]["TipSer"] == RegistroAnterior["TipSer" + cont.toString()] &&
                                ...
                                //$("#tbNivel").jqGrid('setCell', RegistroAnterior["P_inCodNiv"], "dcCan" + cont.toString(), RegistroAnterior["dcCan" + cont.toString()]);
                                break;
                            }
                            cont++;
                        }
...
function CargarNivel() {
    $.ajax({
        url: "Adm_DistribucionMinutosNivel.aspx/ListarColModelNivel", //PageMethod
...
$(function () {
    ...
    function DimPosElementos() {
        Ancho = $(window).width();
        Alto = $(window).height();
        $("#tbNivel").setGridWidth(AnchoGrilla());
        $("#tbNivel").setGridHeight(AltoGrilla());
    }
