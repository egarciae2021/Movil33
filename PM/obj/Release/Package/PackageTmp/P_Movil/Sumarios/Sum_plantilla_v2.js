...
$(function () {
    ...
    var sumarioTipo = $("#hdfTipoSumario").val();

    if (sumarioTipo == "M_NUME") {
        $("#tdTipoLlamada").hide();
        $("#selectServicio").html("");
        $("#selectTipo").html("");
        $("#lblTelefonia").html("Dirección")
        $("#selectServicio").append("<option value=\"Datos\" selected=\"selected\">Datos</option>");
        $("#selectTipo").append("<option value=\"Aplicaciones\" selected=\"selected\">Aplicaciones</option>");
    }
    ...
    $.ajax({
        ...
        success: function (resultado) {
            ...
            if (resul[0] == "1") {
                ...
                console.log("columnas b", columnas);
            ...
            if (resul[0] == "1") {
                ...
                try {
                    if (columnas.length == 11) {
                        columnas[1].width = "375"; //Nombre
                    }
                    else if (columnas.length == 6) {
                        columnas[1].width = "825"; //Nombre
                    }
                    else if (columnas.length == 5) {
                        columnas[1].width = "730"; //Nombre
                    }
                    else if (columnas.length == 7) {
                        columnas[1].width = "535"; //Nombre
                    }
                } catch (e) {
                }

                //console.log("columnas c", columnas);

                columnas = columnas.filter(x => x.name != "FIJA" && x.name != "CEL" && x.name != "DDI");


                var sumarioTipo = $("#hdfTipoSumario").val();
                if (sumarioTipo == "M_NUME") {
                    columnas = columnas.filter(x => x.name != "CÓDIGO");
                    columnas[0].width = "735"; //Nombre
                }
...
function CargarGrafico(datosChart2) {
    ...
    try {
        jsDatos.chart.xaxisname = "Aplicaciones (MB)";
    } catch (e) {
    }
    myChart.setJSONData(jsDatos);
    myChart.setTransparent(true);
...
function fnVistaTotal() {
    ...
    //} else {
    //    $("#selectServicio").append("<option value=\"Datos\" selected=\"selected\">Datos</option>");
    //    $("#selectTipo").append("<option value=\"Cantidad\" selected=\"selected\">Cantidad</option>");
    //}
...
function fnVistaLlamadas() {
    ...
    //$("#filaServicio").show();
    $("#dvUnidad").show();
    $("#dvUnidad").text("(*) Duración expresada en minutos");
