...
$(function () {
    ...
    //setTimeout(function () {
    //    $('iframe').contents().find('body').bind('mouseup', function () {
    //        try {
    //            window.top.window_mouseup();
    //        } catch (e) {
    //        }
    ...
    setInterval(function () {
        ...
        $.ajax({
            type: "POST",
            url: "Common/WebService/General.asmx/ComprobarConexion2",
            //url: "P_Movil/Dashboard/Dashboard_Facturacion.aspx/VerificaSesion",
            //url: "Login.aspx/VerificaSesion",
            data: "{}",
            contentType: "application/json; charset=utf-8",
    ...
    function fn_flag_contrasena(flag) {
        ...
        // ===================================================================================================================================================
        {
            ...
            alerta("La contraseña fue actualizada con éxito");
    ...
    if (window.top.$("#hdf_Reiniciar_Clave").val() == "1" && hdfObtieneMO360 != "1") {
    ...
    var IdProducto = $("#hfIdProducto").val();
    AgregarTabInicial(IdProducto);
...
function CargaInicialSistema(Metodo, Data, fncSatisfactoria, fncError) {
    $.ajax({
        ...
        dataType: "json",
        success: function (result) {
            if (fncSatisfactoria != null && fncSatisfactoria != undefined) { fncSatisfactoria(result.d); }
        },
        error: function (xhr, err, thrErr) {
            if (fncError != null && fncError != undefined) { fncError(xhr, err, thrErr); }
        }
    });
}
...
function fnMyTimerTemporizador() {
    if (Seg_temporizador.TiempoInicial == 0) {
        ...
        window.top.$("#dvTimer").text("" + "0".toHHMMSS());
        ...
        window.top.$("#dvTimer").text("" + Seg_temporizador.TiempoInicial.toString().toHHMMSS());
    }

    Seg_temporizador.TiempoTotalTranscurrido += 1;

    if (Seg_temporizador.TiempoInicial < 20) {
        window.top.$("#dvTimer").css("color", "#FCCED5");
        window.top.$("#dvTimer").css("font-weight", "500");
        window.top.$("#dvTimer").css("font-size", "24px");
        window.top.$("#dvTimer").css("margin-top", "10px");
    }
    else {
        window.top.$("#dvTimer").css("color", "#FAE263");
        window.top.$("#dvTimer").css("font-weight", "500");
        window.top.$("#dvTimer").css("font-size", "24px");
        window.top.$("#dvTimer").css("margin-top", "10px");
    }
...
function alertaGlobal(contenido, Titulo, fnCerrar) {
    ...
    if (Titulo == null || Titulo == undefined) { Titulo = "Mensaje del sistema"; }
...
function DimensionarTab(Ancho, Alto) {
    ...
    $(".ifContenido").css({ height: Alto - 140 }); //130 134
...
function AgregarTabInicial(_IdProducto) {
    var Id = "";
    var Titulo = "";
    //debugger;
    switch (_IdProducto) {
        case "2":
            Id = "#abcdef123456"; //titulo.attr("nombre");                        
            Titulo = "Inicio";
            pagina = "P_Movil/BienvenidaMantenimiento.aspx";
            break;
        case "3":
            if (IdPaginaTab == undefined) {
                try {
                    if (window.top.$("#mainnav-menu").find("#opcion_239").length > 0) {
                        Id = "#opcion_239"; //titulo.attr("nombre");    #Menu_Opcion_239
                        Titulo = "Facturación";
                        pagina = "P_Movil/DashBoard/DashBoard_Facturacion.aspx";
                    }
                    else if (window.top.$("#mainnav-menu").find("#opcion_228").length > 0) {
                        Id = "#opcion_228"; //titulo.attr("nombre");    #Menu_Opcion_239
                        Titulo = "Usuario";
                        pagina = "P_Movil/DashBoard/DashBoard_Resumen.aspx?vcTab=&inCod=228&inTip=4&inTipOri=1";
                    }
                    else {
                        if (window.top.$("#mainnav-menu").find("#modulo_4").length > 0) {
                            Id = "#modulo_4"; //titulo.attr("nombre");    #Menu_Opcion_239
                            Titulo = "Solicitudes";
                            pagina = "P_Movil/Administrar/Solicitudes/Adm_ListadoSolicitudes.aspx?vcTab=&inCod=4&inTip=2&inTipOri=1";
                        } else {
                            id = "";
                        }
                ...
                if (window.top.$("#mainnav-menu").find(IdPaginaTab).length > 0) {
                    Id = IdPaginaTab
                    Titulo = TituloTab;

                    //SE AGREGA ESTO, YA QUE EL MÓDULO DE SOLICITUDES SE NECESITA PARÁMETROS ADICIONALES PARA PODER ABRIRLO.
                    if (IdPaginaTab.indexOf("#modulo_4") > -1) {
                        pagina = "P_Movil/Administrar/Solicitudes/Adm_ListadoSolicitudes.aspx?vcTab=&inCod=4&inTip=2&inTipOri=1";;
                    }
                    else if (IdPaginaTab.indexOf("#opcion_228") > -1) {
                        pagina = "P_Movil/DashBoard/DashBoard_Resumen.aspx?vcTab=&inCod=228&inTip=4&inTipOri=1";
                    }
                    else {
                        pagina = URLTab;
    ...
    var tab = $("#tbPrincipalProducto");
    var $panel = tab.find(Id);
    if (Id != null && Id != "" && !$panel.length) { //En el caso que no exista el tab, lo crea     
        ...
        // Ocultar icon cerrar primer tab
        var primerTab = $(tab.tabs()[0]).find("span");
        primerTab.hide();
    }
    else { //En el caso que exista lo muestra
        tab.tabs('select', Id);
...
function fnAbrirTab(Id, Titulo, Url) {
    if (Url == null || Url == "") {
        return;
    }
