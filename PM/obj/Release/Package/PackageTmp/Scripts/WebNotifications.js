(function (modName, definition) {
    ...
    var _createNewWebNotification = function (title, msg, icon, tag, timeout, fnClick) {
    ...
    var _showWebNotification = function (title, msg, icon, tag, timeout, fnClick) {
        if (_getWebNotificationsSupported()) {
            switch (_getWebNotificationPermissionStatus()) {
                case _WEBNOTIF_PERMISSION_GRANTED:	//Simply show the notification
                    _createNewWebNotification(title, msg, icon, tag, timeout, fnClick);
                    break;
