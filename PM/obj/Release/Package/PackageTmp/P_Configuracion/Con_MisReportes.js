...
$(function () {
    ...
    tbSolicitud = $("#tbSolicitud").jqGrid({
        ...
        sortname: "codreporte", //Default SortColumn
    ...
    function MostrarReporte() {
        ...
        if (id) {
            ...
            if (TipoPivot == 'Llamadas') {
                window.parent.location.href = 'PivotReporte.aspx?codreporte=' + codreporte;
            }
            else if (TipoPivot == 'Organizacion_General') {
                window.parent.location.href = 'PivotReporteOrganizacional_General.aspx?codreporte=' + codreporte;
            }
            else if (TipoPivot == 'Organizacion_Plan') {
                window.parent.location.href = 'PivotReporteOrganizacional_PorPlan.aspx?codreporte=' + codreporte;
            }
            else {
                window.parent.location.href = 'PivotReporteOrganizacional.aspx?codreporte=' + codreporte;
            }
            return true;
        }
        return false;
    ...
    function ListarReportes() {
        $.ajax({
            ...
            data: "{'vcCodEmp': '" + $("#hdfCodEmp").val() + "','TipoPivot':'" + TipoPivot + "'}",
            ...
            success: function (result) {
                ...
                for (i = 0; i < $(result.d).length; i++) {
                    $("#tbSolicitud").jqGrid('addRowData', result.d[i].codreporte, result.d[i]);
                }
            },
                        ...
                        $.ajax({
                            ...
                            success: function (result) {
                                if (result.d == 1) {
                                    ...
                                    //alerta("Registro no pudo ser eliminado.", document, CerroMensaje);
