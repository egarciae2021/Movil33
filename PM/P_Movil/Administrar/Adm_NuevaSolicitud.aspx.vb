...
Partial Class P_Movil_Administrar_Adm_NuevaSolicitud
    ...
    Protected Sub form1_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles form1.Load
        Try
            ...
            Else
                ...
                If Request.QueryString("EsModal") IsNot Nothing Then hdfEsModal.Value = Request.QueryString("EsModal") Else hdfEsModal.Value = "0"
                If Request.QueryString("DefTipoSolicitud") IsNot Nothing Then hdfDefTipoSolicitud.Value = Request.QueryString("DefTipoSolicitud")
                If Request.QueryString("EsCulminada") IsNot Nothing Then hdfEsCulminada.Value = Request.QueryString("EsCulminada")

                hdfSolicitudMultipleEspecialista.Value = Session("SolicitudesMultipleEspecialista").ToString()

                hdfCodLinTip_X_User.Value = ObtenerTipoLinea_X_Usuario(oUsuario)
                hdfCodEmpleado.Value = oUsuario.Empleado.P_vcCod
                hdfCodEmpleadoAutenticado.Value = oUsuario.Empleado.P_vcCod
                hdfAdmin.Value = "0"
                ...
                hdfMostrarAgregarEmpleado.Value = "0"
                ...
                If hdfAdmin.Value = "1" Then
                    ...
                    hdfCodIntEmp.Value = ""
                    hdfMostrarAgregarEmpleado.Value = "1"

                ElseIf hdfJefeArea.Value = "1" Or oUsuario.CodIntResp <> "" Then 'Responsable de por lo menos un área
                    ...
                    Else 'Usuario de tipo organización
                        Dim lstArea = oUsuario.CodIntResp.Split(",")
                        If lstArea.Count > 1 Then
                            For i As Integer = 0 To lstArea.Length - 1
                                vcWhere += vcWhere + " EMPL_CodInt2 LIKE |" + lstArea(i) + "%| or "
                            Next
                        Else
                            vcWhere = vcWhere + "EMPL_CodInt2 LIKE |" + oUsuario.CodIntResp + "%| or "
                        End If
                    End If
                ...
                Else
                    If oUsuario.Empleado.P_vcCod <> "" Then
                        ...
                        hdfCodEmpleado.Value = oUsuario.Empleado.P_vcCod
                        hdfCodIntEmp.Value = oUsuario.CodIntResp 'F_vcCodInt
                    End If
    ...
    <WebMethod()>
    Public Shared Function ListarTipoSolicitudXGrupoEmpleado(ByVal vcCodEmp As String,
                                                             ByVal vcTipLin As String,
                                                             ByVal vcJefeArea As String) As List(Of ENT_MOV_SolicitudTipo)
        Try
            ...
            Dim oUsuario As ENT_SEG_Usuario = CType(HttpContext.Current.Session("Usuario"), ENT_SEG_Usuario)
            Dim _returnFiltrado As List(Of ENT_MOV_SolicitudTipo) = New List(Of ENT_MOV_SolicitudTipo)

            If vcJefeArea <> "1" And oUsuario.F_vcCodInt = "" Then 'si no es Responsable de área
                For Each oSolicitud As ENT_MOV_SolicitudTipo In _return
                    If oSolicitud.IntCategoria = 2 Then 'si la categoria es General
                        _returnFiltrado.Add(oSolicitud)
                    End If
                Next
            Else
                _returnFiltrado = _return
            End If

            Solicitud.Dispose()
            Return _returnFiltrado
        Catch ex As Exception
            Dim util As New Utilitarios
            util.GrabarLog(ex, HttpContext.Current.Server.MapPath("~/"), Utilitario.NombreSistemaMovil, HttpContext.Current.Session("Usuario"))
            Throw New Exception(UtilitarioWeb.MensajeError)
    ...
    Public Shared Function ListarContenidos(ByVal inTipSol As Integer) As List(Of Dictionary(Of String, String))
        ...
        Dim dict As List(Of Dictionary(Of String, String)) = Solicitud.ListarContenidoTabs(inTipSol, vcFilePath)
        Solicitud.Dispose()
        Return dict
    End Function
    ...
    Public Shared Function EnviarSolicitudPersonalizada(ByVal vcCodEmp As String, ByVal inTipSol As String, ByVal vcCamPer As String, ByVal vcValPer As String, ByVal vcAuditoria As String, _
        ...
        Try
            ...
            If dcMonto = "" Then dcMonto = 0
            If vcMeses = "" Then vcMeses = "0"
            ...
            "Envío de solicitud personalizada", "Agregar", "USUARIO: " & oAuditoria.NombreUsuario & ". MÓDULO: " & oAuditoria.Modulo & ".TABLA: " & oAuditoria.Tabla)
        ...
        Try
            ...
            Else
                ...
                If resultado >= 0 Then
                    ...
                    Utilitario.RegistrarLog(PCSistelMovilLog45.LogBE.eNivel.Auditoria,
                    "Envío de solicitud personalizada como culminada", "Agregar", "USUARIO: " & oAuditoria.NombreUsuario & ". MÓDULO: " & oAuditoria.Modulo & ".TABLA: " & oAuditoria.Tabla)
    ...
    Private Shared Sub AuditoriaCrear(ByVal codSol As String, ByVal oUsuario As ENT_SEG_Usuario, ByVal vcTabla As String)
        ...
        Utilitario.RegistrarLog(PCSistelMovilLog45.LogBE.eNivel.Auditoria,
        "Nueva solicitud: " + vcTabla, "Agregar", "USUARIO: " & oAuditoria.NombreUsuario & ". MÓDULO: " & oAuditoria.Modulo & ".TABLA: " & oAuditoria.Tabla)
