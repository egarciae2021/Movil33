...
Partial Class P_Movil_Administrar_Cam_DespachoOperadorOS
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        Try
            If Not IsPostBack Then
                ...
                Dim oLista As List(Of ENT_MOV_ModeloDispositivo) = ListarOrdenServicio(100, "")
                Dim oENT_MOV_ModeloDispositivo As New ENT_MOV_ModeloDispositivo
                oENT_MOV_ModeloDispositivo.P_inCod = -1
                oENT_MOV_ModeloDispositivo.vcNom = "(Seleccione)"
                oLista.Insert(0, oENT_MOV_ModeloDispositivo)
                UtilitarioWeb.Dataddl(ddlOrdenServicio, oLista, "vcNom", "P_inCod")
    ...
    <WebMethod()>
    Public Shared Function ObtenerSolicitudes(ByVal IdOrden As String) As List(Of Object)
        ...
        Try
            ...
            Dim dsDatos As DataSet = Solicitud.OrdenServicio_Almacen("1", IdOrden)

            ''Validar Adjunto Factura....
            If dsDatos.Tables.Count > 1 AndAlso dsDatos.Tables(1).Rows.Count > 0 Then
                Dim Archivo As String = dsDatos.Tables(1).Rows(0)("NombreAdjuntoFactura").ToString()
                If Archivo <> "" Then
                    Dim CarpetaDominio As String = ""
                    If ConfigurationManager.AppSettings("ModoCloud").ToString() = "1" Then
                        CarpetaDominio = UtilitarioWeb.ObtenerCarpetaDominio(HttpContext.Current.Server.MapPath("~") + "/P_Movil/Administrar/Plantillas/", "/")
                    ...
                    End If
                    Dim strDirectorio As String = HttpContext.Current.Request.MapPath("~/Images/Temporal/" + CarpetaDominio) & "/"
                    Dim strfn As String = Path.Combine(strDirectorio, Archivo)
                    If File.Exists(strfn) Then File.Delete(strfn)
                    File.WriteAllBytes(strfn, CType(dsDatos.Tables(1).Rows(0)("FacturaPdf"), Byte()))
                End If
            End If

            Return ConvertirDataSet_ListaObjetos(dsDatos)
    ...
    <WebMethod()>
    Public Shared Function GrabarIngreso_Nuevos(ByVal ListaIngresoNuevo As String,
                                                ByVal EsRenovacion As String,
                                                ByVal Operador As String,
                                                ByVal NroFactura As String, ByVal FecFactura As String,
                                                ByVal MonFactura As String, OrdenServicio As String,
                                                ByVal nomArch As String) As Object

        If FecFactura.Contains("/") Then
            FecFactura = FecFactura.Substring(6, 4) & FecFactura.Substring(3, 2) & FecFactura.Substring(0, 2)
        End If

        Dim Respuesta As New Dictionary(Of String, Object)
        Dim serial As JavaScriptSerializer = New JavaScriptSerializer()
        Dim ListaIngreso As List(Of Dictionary(Of String, Object)) = serial.Deserialize(Of List(Of Dictionary(Of String, Object)))(ListaIngresoNuevo)

        Dim Campana As String, pruta As String, LineaTipo As String, Linea As String, ICC As String, _
            IdModeloDispositivo As String, ModeloDispositivo As String, IMEI As String, Estado As String, Guia As String,
            Observacion As String, NroCuenta As String, FecIniContrato As String, MesesContrato As Integer, _
            IdPlanLinea As String, PlanLinea As String, DescripcionModelo As String, Serie As String
        ...
        Try
            ...
            For Each Registro As Dictionary(Of String, Object) In ListaIngreso
                Linea = Registro("Linea_Numero")
				ICC = Registro("Linea_ICC")
                MesesContrato = Registro("Linea_MesesContrato")
                IMEI = Registro("Dispositivo_IMEI")
                IdModeloDispositivo = Registro("Dispositivo_IdModelo")
                ModeloDispositivo = Registro("Dispositivo_Modelo")
                NroCuenta = Registro("Linea_Cuenta")
                Serie = Registro("Dispositivo_Serie")
                FecIniContrato = Registro("Linea_FechaInicio")

                'If FecIniContrato.Contains("/") Then
                '    FecIniContrato = FecIniContrato.Substring(6, 4) & FecIniContrato.Substring(3, 2) & FecIniContrato.Substring(0, 2)
                'End If

                DescripcionModelo = Registro("Observacion")
                Guia = ""
                ...
                dr("IdDespachoOperadorDetalle") = ""
                dr("IdDespachoOperador") = ""
                dr("IdSolicitud") = Registro("P_inCodSol")
                dr("Linea") = If(Linea.ToString().Trim() = "", DBNull.Value, Linea.ToString().Trim())
				dr("ICC") = If(ICC.ToString().Trim() = "", DBNull.Value, ICC.ToString().Trim())
                dr("IdModeloDispositivo") = If(IdModeloDispositivo.ToString().Trim() = "", DBNull.Value, Convert.ToInt32(IdModeloDispositivo))
                dr("ModeloDispositivo") = If(ModeloDispositivo.ToString().Trim() = "", DBNull.Value, ModeloDispositivo.ToString().Trim())
                dr("IncluyeSIMCard") = If(Linea = "", "False", "True")
                dr("IMEI") = If(IMEI.ToString().Trim() = "", DBNull.Value, IMEI.ToString().Trim())
                dr("SerieSIMCard") = DBNull.Value
                ...
                dr("EsRenovacion") = EsRenovacion
                dr("NroCuenta") = If(NroCuenta.ToString() = "-1", DBNull.Value, NroCuenta.ToString())
                dr("FecIniContrato") = If(FecIniContrato.ToString() = "", DBNull.Value,
                                          New DateTime(FecIniContrato.Substring(6, 4), FecIniContrato.Substring(3, 2), FecIniContrato.Substring(0, 2)))
                dr("btEstProc") = "False"
                dr("vcObsProc") = DBNull.Value
                dr("MesesContrato") = If(MesesContrato = -1, DBNull.Value, MesesContrato)
                dr("IdPlanLinea") = DBNull.Value
                dr("PlanLinea") = DBNull.Value
                dr("vcDesMod") = If(DescripcionModelo.ToString().Trim() = "", DBNull.Value, DescripcionModelo.ToString().Trim())
                dr("Serie") = If(Serie.ToString().Trim() = "", DBNull.Value, Serie.ToString().Trim())
                dt.Rows.Add(dr)
            ...
            If File.Exists(strfn) Then
                ...
                ''File.Delete(strfn)
            End If
            If Trim(MonFactura) = "" Then MonFactura = "0"

            Dim dsResult As New DataSet()
            dsResult = Despacho.GuardarIngresoAlmacen(dt, Operador, 1, -1, "Nuevo", pruta, NroFactura,
                                                      FecFactura, Convert.ToDecimal(MonFactura), OrdenServicio, adjunto)
            ''EnviarCorreosRenovacion(dsResult.Tables(2))

            resultado = ""
            For Each Fila As DataRow In dsResult.Tables(1).Rows
                resultado &= ";" & Fila("IdSolicitud").ToString() & "|" & Fila("descError").ToString()
            Next
            If resultado.Length > 0 Then resultado = resultado.Substring(1)
            Respuesta.Add("ResultadoErr", resultado)

            resultado = ""
            For Each Fila As DataRow In dsResult.Tables(0).Rows
                resultado &= ";" & Fila("IdSolicitud").ToString() & "|"
            Next
            If resultado.Length > 0 Then resultado = resultado.Substring(1)
            Respuesta.Add("ResultadoOK", resultado)
            Respuesta.Add("Error", "")
        Catch ex As Exception
