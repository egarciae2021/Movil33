...
function CargarDatos() {
    $.ajax({
        url: "Adm_DistribucionMinutosGrupoOrigen.aspx/ListarGrupoOrigen", //PageMethod
        data: "{ 'inPagTam':'" + $("#tbGrupoOrigen").getGridParam("rowNum") + "'," + //Tamaño de pagina
                        "'inPagAct':'" + parseInt($("#tbGrupoOrigen").getGridParam("page")) + "'," + //Pagina actual
        ...
        success: function (result) {
            $("#tbGrupoOrigen")[0].addJSONData(result.d);
        },
...
function CargarGrillaGrupoOrigen() {
    $("#tbGrupoOrigen").jqGrid({
        datatype: CargarDatos,
        colModel: colModelGrupoOrigen,
        shrinkToFit: false,
        height: AltoGrilla(),
        width: AnchoGrilla(),
        rownumbers: true,
        ...
        onSelectRow: function (id) {
            ...
            RegistroActual = $("#tbGrupoOrigen").jqGrid('getRowData', id);
            ...
            var editparameters = {
                ...
                "oneditfunc": null,
                "successfunc": null,
                "url": "Adm_DistribucionMinutosGrupoOrigen.aspx/GuardarServicios",
                "extraparam": {},
                "aftersavefunc": function (id, resp) { GuardarServiciosGrupoOrigen(id, resp); },
            ...
            if (id && id !== lastsel) {
                $("#tbGrupoOrigen").saveRow(lastsel, editparameters);
                $("#tbGrupoOrigen").editRow(id, true);
                lastsel = id;
            }
        },
        subGrid: true,
    ...
    }).navGrid("#pagerGrupoOrigen", { edit: false, add: false, search: false, del: false });
    //$("#tbGrupoOrigen").jqGrid('setFrozenColumns');
}

function GuardarServiciosGrupoOrigen(id, resp) {
    var registro = $("#tbGrupoOrigen").jqGrid('getRowData', id);

    $.ajax({
        url: "Adm_DistribucionMinutosGrupoOrigen.aspx/GuardarServicios", //PageMethod
        data: "{'registro':'" + JSON.stringify(registro) + "'," +
                       "'vcValIli':'" + $("#hdfValorIlimitado").val() + "'," + //
                    ...
                    for (i in result.d.lstResultado) {
                        ...
                        while (RegistroAnterior["TipSer" + cont.toString()] != undefined) {
                            if (result.d.lstResultado[i]["TipSer"] == RegistroAnterior["TipSer" + cont.toString()] &&
                                ...
                                $("#tbGrupoOrigen").jqGrid('setCell', RegistroAnterior["P_inCodGruOri"], "dcCan" + cont.toString(), RegistroAnterior["dcCan" + cont.toString()]);
                                break;
                            }
                            cont++;
                        }
...
function CargarGrupoOrigen() {
    $.ajax({
        url: "Adm_DistribucionMinutosGrupoOrigen.aspx/ListarColModelGrupoOrigen", //PageMethod
...
$(function () {
    ...
    function DimPosElementos() {
        Ancho = $(window).width();
        Alto = $(window).height();
        $("#tbGrupoOrigen").setGridWidth(AnchoGrilla());
        $("#tbGrupoOrigen").setGridHeight(AltoGrilla());
    }
