...
Public Class Comp_ExportacionCobros
        ...
        Try
            ...
            Else
                ...
                If Not IsPostBack Then
                    ...
                    Dim dsDetalle = ConfiguracionDetalle.MostrarExportacionCobro(codigo)
    ...
    Public Sub CargarDatosEquipo(ByVal dtComprobanteEquipo As DataTable, ByRef lstConfiguracionConcepto As List(Of ENT_MOV_FAC_ConfiguracionConcepto))
        Dim objControlFinder As New ControlFinder

        For i As Integer = 0 To dtComprobanteEquipo.Rows.Count - 1
            If (dtComprobanteEquipo.Rows(i)("Vigente") = True) Then
                Dim trFila As HtmlTableRow = CType(objControlFinder.FindControlRecursive(Me, "trComprobanteEq" + (i + 1).ToString()), HtmlTableRow)
                Dim ddlDiasAntes As DropDownList = CType(objControlFinder.FindControlRecursive(Me, "ddlDiasAntesEq" + (i + 1).ToString()), DropDownList)
                Dim ddlTipoConcepto As DropDownList = CType(objControlFinder.FindControlRecursive(Me, "ddlTipoConceptoEq" + (i + 1).ToString()), DropDownList)
                Dim ddlDiaCalendario As DropDownList = CType(objControlFinder.FindControlRecursive(Me, "ddlDiaCalendarioEq" + (i + 1).ToString()), DropDownList)
                Dim txtHoraEjecucion As TextBox = CType(objControlFinder.FindControlRecursive(Me, "txtHoraEjecucionEq" + (i + 1).ToString()), TextBox)
                ...
                ddlDiaCalendario.SelectedValue = If(dtComprobanteEquipo.Rows(i)("EsDiaCalendario") = False, 0, 1)

                ddlTipoConcepto.SelectedValue = dtComprobanteEquipo.Rows(i)("IdConfiguracionConcepto")
                If (dtComprobanteEquipo.Rows(i)("IdConfiguracionConcepto").ToString() = 1) Then
                    Dim ddlDiaConcepto As DropDownList = CType(objControlFinder.FindControlRecursive(Me, "ddlDiaConceptoEq" + (i + 1).ToString()), DropDownList)
                    ddlDiaConcepto.SelectedValue = dtComprobanteEquipo.Rows(i)("DiaConcepto")
                End If

                txtHoraEjecucion.Text = Convert.ToDateTime(dtComprobanteEquipo.Rows(i)("HoraEjecucion")).ToString("hh:mm")
...
<ScriptMethod(ResponseFormat:=ResponseFormat.Json)> _
    ...
    <WebMethod()>
    Public Shared Function Insertar_ConfExportCobros(ByVal IdFacConfiguracion As Integer, ByVal TipoEjecucion As String, ByVal IdConfigProceso_Origen As Integer, ByVal IdConfigProceso_Destino As Integer, ByVal XML_CfgDetalle As String, CorreoNotificacion As String) As Integer
        Dim oUsuario As ENT_SEG_Usuario = CType(HttpContext.Current.Session("Usuario"), ENT_SEG_Usuario)
        Dim ConfiguracionDetalle As BL_MOV_FAC_ConfiguracionDetalle = Nothing
        ...
        Try
            ...
            'Return 0
            Return ConfiguracionDetalle.ActualizarExportacionCobro(IdFacConfiguracion, IdConfigProceso_Origen, IdConfigProceso_Destino, XML_CfgDetalle, CorreoNotificacion, UsuarioModificacion)
