...
Partial Class P_Movil_Facturacion_Sumarios_Sum_plantilla
        ...
        Try
            ...
            Else
                If Not IsPostBack Then
                    ...
                    Dim codigosAreaAsiganadas = vcCodInt.Split(",")
                    Dim codigoMasAlto As String = ""
                    If codigosAreaAsiganadas.Length > 1 Then
                        For Each codioArea In codigosAreaAsiganadas
                            If codigoMasAlto = "" Then
                                codigoMasAlto = codioArea
                            End If

                            If codigoMasAlto.Length > codioArea.Length Then
                                codigoMasAlto = codioArea
                            End If
                        Next
                    Else
                        vcCodInt = codigoMasAlto
                    End If
                    ...
                    Dim inNivel As Integer = codigoMasAlto.Length / 3
                    Dim oUltimoNivel As New ENT_GEN_Nivel
                    For x As Byte = 2 To inNivel
                        If x = inNivel Then
                            oUltimoNivel = oNivel.Item(0)
                        End If
                        oNivel.Remove(oNivel.Item(0))
                    Next
                    ...
                    Facturacion = New BL_MOV_IMP_Facturacion(oUsuario.IdCliente)

                    Dim js As New JavaScriptSerializer()
                    Dim script As String = "var misTiposServicios = " + js.Serialize(Facturacion.ObtieneGrupoConceptoxMostrarDashboardSumario()) + ";"
    ...
    Public Shared Function obtenerSumario(ByVal prNomtabla As String, ByVal prTipo As String, ByVal prTipoConsulta As String, ByVal prTop As String _
        ...
        Try
            ...
            If (vcCodInt.Contains(",")) Then
                ...
                Dim arrayCodigosAreas As String() = vcCodInt.Split(",")

                For Each codigo As String In arrayCodigosAreas
                    'C.CodigoOrganizacion like ''' + @vcCodInt + '%'' OR ISNULL(C.CodigoOrganizacion,'''') = ''''
            ...
            oMostrarSumario.FiltroCodigoArea = FiltroCodSucursalesParaCuentas
            ...
            oMostrarSumario.P_Desc = prP_Desc.Replace("[", "+' ('+").Replace("]", "+')'")
            oMostrarSumario.F_Desc = prF_Desc.Replace("[", "+' ('+").Replace("]", "+')'")
    ...
    <WebMethod()>
    Public Shared Function obtenerConceptosPorGrupo(ByVal prIdTipoGrupo As Integer, ByVal pTipo As String) As List(Of ENT_PCS_MOV_ServicioResumen) 'obtenerServiciosPorTipoServicio

        Dim Facturacion As BL_MOV_IMP_Facturacion = Nothing
        Try
            Facturacion = New BL_MOV_IMP_Facturacion(CType(HttpContext.Current.Session("Usuario"), ENT_SEG_Usuario).IdCliente)
            Return Facturacion.ObtieneServicioResumenPorGrupo(prIdTipoGrupo, pTipo)
        Catch ex As Exception
            Dim util As New Utilitarios
            util.GrabarLog(ex, HttpContext.Current.Server.MapPath("~/"), Utilitario.NombreSistemaMovil, HttpContext.Current.Session("Usuario"))
            Throw New Exception(UtilitarioWeb.MensajeError)
