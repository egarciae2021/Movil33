        ...
        success: function (result) {
            ...
            if (window.parent.ChatActivo == true && window.parent.socket != null) {
                ...
                if (strRutaImagenEstado.indexOf("estadoInactivo.png") >= 0) {
                    ...
                    window.parent.socket.emit("EnviarMensajeUsuarioNoLeido", tickets[index].Usuario.vcUsu + '|' + DescripcionUsuario + '|' + tickets[index].Asunto.toString() + '|' + tickets[index].P_inCod.toString() + '|' + ObtieneFechaGeneral() + '-' + ObtieneHoraGeneral() + '-' + ObtieneNumeroAleatorio());
                    $("#img_usuario_" + tickets[index].Usuario.P_inCod).attr("src", "../../Common/Images/Chat/estadoContestado.png");
                }
                else {
                    window.parent.socket.emit("EnviarMensajeUsuario", tickets[index].Usuario.vcUsu + '|' + DescripcionUsuario + '|' + descripcion + '|' + tickets[index].P_inCod.toString() + '|' + ObtieneFechaGeneral() + '-' + ObtieneHoraGeneral() + '-' + ObtieneNumeroAleatorio());
                    $("#img_usuario_" + tickets[index].Usuario.P_inCod).attr("src", "../../Common/Images/Chat/estadoContestado.png");
                }
            }
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            for (var i = 0; i < $(result.d).length; i++) {
                ...
                $('#pnlDetalle').append('<div class="msm" style="width: 99%; text-align: left; overflow:auto; border-bottom:1px solid white; word-wrap: break-word;"> <p> <span  style="font-size:10px; font-weight: bold;">' + result.d[i].IdUsuario.toString() + ' dice (' + Fecha + ' - ' + result.d[i].HoraRegistro.toString() + '): </span><span  style="font-size:9pt;"> ' + result.d[i].Descripcion.toString() + '</p></div>');
...
function fnUsuariosConectados(data) {
    ...
    for (i in mUsuarios) {
        ...
        oUsuario.vcUsu = mUsuarios[i].split("-")[1];
...
function fnAgregarMensaje(vIdUsuario, vNombreUsuario, vMensaje, vIdTicket) {
    ...
    if (CodigoItemActualSeleccionado == vIdTicket) {
        $('#pnlDetalle').append('<div class="msm" style="width: 99%; text-align: left; overflow:auto; border-bottom:1px solid white; word-wrap: break-word;"> <p> <span  style="font-size:10px; font-weight: bold;">' + vNombreUsuario + ' dice (' + vFecha + ' - ' + vHora + '): </span><span  style="font-size:9pt;"> ' + fnVerificarEmoticones(vMensaje) + '</p></div>');
...
$.timer(700, function (temporizador) {
    for (var i = 0; i < $(".focusticket").length; i++) {
    ...
    var idUsuario = '';
    for (var i = 0; i < $(".focusUsuario").length; i++) {
