...
public class UploadHandler : IHttpHandler
{
    ...
    {
        string strCarpetaDominio = context.Request.QueryString["CarpetaDominio"].ToString();
        
        //Uploaded File Deletion
        if (context.Request.QueryString.Count > 1)
        {
            string filePath = HttpContext.Current.Server.MapPath("DownloadedFiles") + strCarpetaDominio + "//" + context.Request.QueryString[0].ToString();
        ...
        else
        {
            ...
            fileName = GetUniqueFileName(fileName, HttpContext.Current.Server.MapPath("DownloadedFiles" + strCarpetaDominio + "/"), ext).ToLower();
            string location = HttpContext.Current.Server.MapPath("DownloadedFiles" + strCarpetaDominio + "/");
            string archivo = location + fileName + ext;
            
            //verificar existencia de locacion
            if (!Directory.Exists(location))
                Directory.CreateDirectory(location);
            
            context.Request.Files[0].SaveAs(archivo);
