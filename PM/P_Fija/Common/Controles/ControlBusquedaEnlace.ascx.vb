...
Imports System.Text
Imports GeneralMantenimiento

Partial Class ControlBusquedaEnlace
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            ...
            If Not IsPostBack Then
                If _anchoDialogo = 0 Then
                    _anchoDialogo = 800
                End If
                If _altoDialogo = 0 Then
                    _altoDialogo = 500
                End If
                If _anchoCaja = 0 Then
                    _anchoCaja = 308
                End If
...
#Region "Genera Scripts Cliente"
    ...
    Private Function ObtieneScript_btnBuscar_Click() As String
        ...
        If _esClicleabe Then
            ...
            sbScript.AppendLine((Convert.ToString("   var $Pagina = raiz('") & _UrlPagina) + "' + vcCodigo);")
            sbScript.AppendLine((Convert.ToString("   ") & _UbicacionWindow) + ".$('#iframe_modal').width($width - 10);")
            sbScript.AppendLine((Convert.ToString("   ") & _UbicacionWindow) + ".$('#iframe_modal').height($height - 30);")
            sbScript.AppendLine((Convert.ToString("   ") & _UbicacionWindow) + ".$('#iframe_modal').attr('src', $Pagina);")
            sbScript.AppendLine((Convert.ToString("   var dlgOrganizacion = ") & _UbicacionWindow) + ".$('#div_modal').dialog({")
            sbScript.AppendLine((Convert.ToString("       title: '") & _TituloDialog) + (If(Not String.IsNullOrEmpty(_Descripcion), Convert.ToString(" - ") & _Descripcion, "")) + "',")
...
#End Region
    ...
    Private Function ObtieneHtmlControl() As Object
        ...
        If _esClicleabe Then
            sbScript.Append((Convert.ToString("   <span id='" + Me.ClientID + "_spControl' class='lui'>") & _Descripcion) + "</span>")
        Else
            sbScript.Append((Convert.ToString("   <span id='" + Me.ClientID + "_spControl' class='luiNO'>") & _Descripcion) + "</span>")
        End If
        sbScript.Append((Convert.ToString("   <input id='" + Me.ClientID + "_hdControl' type='hidden' value='") & _Codigo) + "' />")
        sbScript.Append("</div>")
