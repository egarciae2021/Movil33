        ...
        if ($("#hdfvcTab").val() == "SEG_Usuario") {
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.d == '3') {
                        $("#btnEliminar").button("option", "disabled", true);
                        $("#btnEliminar").attr("title", "No se puede eliminar. Es técnico responsable de un tipo de solicitud.");
                        //alerta("El usuario no se puede eliminar por que es técnico responsable de un tipo de solicitud.");
                    } else if (result.d == '2') {
                        $("#btnEliminar").button("option", "disabled", true);
                        $("#btnEliminar").attr("title", "No se puede eliminar. Es técnico asignado a una solicitud.");
...
function CambiarRegistro() {
    ...
    if (idSeleccionado == null) {
        if ($($("#grid").jqGrid('getGridParam', 'selarrrow')).length == 1)
            ...
            alerta("Ha seleccionado más de un elemento, seleccione sólo un elemento.");
            return;
        }
    }
    else
        ...
        for (i in ids) {
            ...
            for (valor in ValoresPorDefectoId) {
                if ($.trim(ValoresPorDefectoId[valor]) == $.trim(datos[ids[i]])) {
                    alerta("El registro no puede ser cambiado, es un registro del sistema.");
                    return;
                }
            }
        }
...
function EliminarRegistro() {
    ...
    if ($(idsSel).length > 0)
        ...
        alerta("Seleccione por lo menos un item");
};
...
function IngresarAreas(areas) {//Agrega las areas seleccionadas del formulario respectivo
    ...
    $.ajax({
        type: "POST",
        url: "Adm_Lista.aspx/ObtenerCodigoReporte",
        data: "{'codCondicion':'" + codArea + "'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (result) {
    ...
    if (tabs == "M_EMPL") {
        //pagina = '../../Mantenimiento/Reportes/Adm_Reporte.aspx?vcTab=rptEmpleadoxArea&condicion=' + codArea;
        pagina = '../../Mantenimiento/Reportes/Adm_Reporte.aspx?vcTab=rptEmpleadoxArea' ;
        Titulo = "Imprimir(Jerárquico)"
    }
    if (tabs == "M_ANEX") {
        //pagina = '../../Mantenimiento/Reportes/Adm_Reporte.aspx?vcTab=rptAnexoArea&condicion=' + codArea;
        pagina = '../../Mantenimiento/Reportes/Adm_Reporte.aspx?vcTab=rptAnexoArea';
        Titulo = "Extensión por Área"
    }
    if (tabs == "M_CODI") {
        //pagina = '../../Mantenimiento/Reportes/Adm_Reporte.aspx?vcTab=rptCodigosxArea&condicion=' + codArea;
        pagina = '../../Mantenimiento/Reportes/Adm_Reporte.aspx?vcTab=rptCodigosxArea';

        Titulo = "Códigos por Sucursales"
    }

    //pagina = '../../Mantenimiento/Reportes/Adm_Reporte.aspx?vcTab=rptEmpleadoxArea&condicion=' + codArea;
    var Id = "#" + $("#hdfvcTab").val() + "_Tab_" + $(this).attr("idTab");
    //Titulo = "Seleccionar Area"
    AbrirTab(Id);
  
    //../../Mantenimiento/Reportes/Adm_Reporte.aspx
    //pagina = $(this).attr("URL");
    //$(".aReportes").click();
}

function fnMostrarCodigoEmpleado(valor) {
    var descripcion = $('#bpBusquedaOrganizacion_txtValorBusqueda').val();   
    var tabs = $("#hdfvcTab").val();

    if (tabs == "M_EMPL") 
    {
        pagina = '../../Mantenimiento/Reportes/Adm_Reporte.aspx?vcTab=rptEmpleadoJerarquico&condicion=' + valor;
        Titulo = "Imprimir(Jerárquico)"
    }

    if (tabs == "M_ORGA") {
        pagina = '../../Mantenimiento/Reportes/Adm_Reporte.aspx?vcTab=rptOrganizacionListarNive&condicion=' + valor;
        Titulo = "Imprimir(Nivel)"
    }
    var Id = "#" + $("#hdfvcTab").val() + "_Tab_" + $(this).attr("idTab");  
    AbrirTab(Id);
...
function IngresarSucursales(sucursales,tipo) {
    ...
    $(sucursales).each(function () {
        var Sucursal = this;
        Sucursal.vcCodInt = Sucursal.P_vcCod;
        Sucursal.P_vcCod = Sucursal.vcNom.split("=")[0];
        codSucursal = codSucursal + Sucursal.P_vcCod + ','

    });
    if (codSucursal.length < 1) {
        alerta("Seleccione las Sucursales que desea visualizar");
        return;
    }

    codSucursal = codSucursal.substring(0, codSucursal.length - 1);
    //codSucursal = "01";
    window.Modal.dialog("close");
    ...
    if (tabs == "M_CODI") {
        //pagina = '../../Mantenimiento/Reportes/Adm_Reporte.aspx?vcTab=rptCodigoPorSucFisLog&condicion=' + codSucursal + '&tipo=' + tipo;
        pagina = '../../Mantenimiento/Reportes/Adm_Reporte.aspx?vcTab=rptCodigoPorSucFisLog&tipo=' + tipo;
        Titulo = "Códigos por Sucursales"
    }
