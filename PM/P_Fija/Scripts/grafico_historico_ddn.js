function ArmaGraficoHistoricoDDN(codint) {
    ...
    $.ajax({
        type: "POST",
        dataType: "json",
        data: "{'fecha_consulta':'" + txt_fecha.GetText() + "','codint':'" + codint + "','tipo':'ddncosto', 'telefonia':'" + txt_telefonia.GetText() + "'}",
        contentType: "application/json; charset=utf-8",
        url: "resumen_datos.aspx/getDatosHistoricos",
        success: function (items) {

            if (items.d == "{}") {
                $("#blanco_grafico_historico_DDN").show();
                $("#grafico_historico_DDN").hide();
            }
            else {
                $("#blanco_grafico_historico_DDN").hide();
                $("#grafico_historico_DDN").show();
            }
