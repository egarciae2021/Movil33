...
// This is the ols search Filter method used in navigator.
	...
	filterGrid : function(gridid,p){
		...
		return this.each(function(){
			var self = this;
			this.p = p;
			if(this.p.filterModel.length === 0 && this.p.gridModel===false) { alerta("No filter is set"); return;}
			if( !gridid) {alerta("No target grid is set!"); return;}
			...
			if(gcolMod) {
				...
				alerta("Could not get grid colModel"); return;
			}
