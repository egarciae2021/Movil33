...
Public Class VisorVersionProducto
        ...
        Try
            ...
            Else
                ...
                If Not IsPostBack Then
                    ...
                    'Dim lstCampoHistorico As List(Of ENT_ENT_Campo)
                    'Dim oEntidadHistorico As ENT_ENT_Entidad

                    lstCampo = Campo.Listar("vw_GEN_VersionProducto", oUsuario, 0)
                    oEntidad = Entidad.Mostrar("vw_GEN_VersionProducto", oUsuario.P_inCod)
                    Session("Campos_vw_GEN_VersionProducto") = lstCampo
                    ConfigurarGrid(lstCampo, oEntidad)

                    lstCampo = Campo.Listar("vw_GEN_VersionProductoHistorico", oUsuario, 0)
                    oEntidad = Entidad.Mostrar("vw_GEN_VersionProductoHistorico", oUsuario.P_inCod)

                    Session("Campos_vw_GEN_VersionProductoHistorico") = lstCampo
                    ConfigurarGrid(lstCampo, oEntidad)
    ...
    <WebMethod()>
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    Public Shared Function ListarHistorico(ByVal inPagTam As String, ByVal inPagAct As String, ByVal vcOrdCol As String, ByVal vcTipOrdCol As String, ByVal vcCam As String, ByVal vcValBus As String,
                                  ByVal vcTab As String, ByVal inTipOri As String, ByVal inFilReg As String) As JQGridJsonResponse
        Dim Campo As BL_ENT_Campo = New BL_ENT_Campo(Integer.Parse(inTipOri), CType(HttpContext.Current.Session("Usuario"), ENT_SEG_Usuario).IdCliente)
        Try
            vcTab = "vw_GEN_VersionProductoHistorico"
            inTipOri = "1"
            HttpContext.Current.Session("vcFiltro_" & vcTab) = vcCam & "," & vcValBus & "|" & inFilReg
    ...
    Protected Sub eegVisorVersionProducto_ObtenerDatosAExportar(oTipoExcel As ExportarExcelGenerico.TipoExcel) Handles eegVersionProducto.ObtenerDatosAExportar
        Dim vcTab As String = "vw_GEN_VersionProductoHistorico"
        ...
        Dim Campo As BL_ENT_Campo = New BL_ENT_Campo(Integer.Parse(inTipOri), CType(HttpContext.Current.Session("Usuario"), ENT_SEG_Usuario).IdCliente)
        Dim lstCampo As List(Of ENT_ENT_Campo) = CType(HttpContext.Current.Session("Campos_vw_GEN_VersionProductoHistorico"), List(Of ENT_ENT_Campo))
        Try
            ...
            For Each objCampo In lstCampo
                ...
                End If
            Next
            eegVersionProducto.ExportarDatos(dsDetalle.Tables(0), "TotalRegistros", lstCampo)
        Catch
