...
$(function () {
    ...
    function inicio() {
        ...
        $("#ddlPlantilla").attr("title", "Seleccione un operador y un tipo de archivo para activar esta opción");
    ...
    $("#btnGuardar").click(function () {
        ...
        if ($("#fulArchivo").val() == "" && $("#fulArchivo_Cortes").val() == "" && $("#fulArchivo_Adendum").val() == "") {
            alerta("Debe seleccionar el archivo a procesar en Reporte F y C");
            return;
        }
        //if ($("#fulArchivo_Cortes").val() == "" && $("#fulArchivo_Adendum").val() != "") {
        //    alerta("Debe seleccionar el archivo a procesar en Cortes");
        //    return;
        //}
        //if ($("#fulArchivo_Adendum").val() == "" && $("#fulArchivo_Cortes").val() != "") {
        //    alerta("Debe seleccionar el archivo a procesar en Adendum");
        //    return;
        //}

        var ext = "";
        if ($("#fulArchivo").val() != "") {
            ext = $('#fulArchivo').val().split('.').pop().toLowerCase();
            if ($.inArray(ext, ['xlsx', 'xls']) == -1) {
                alerta('Debe seleccionar archivos de tipo Excel');
                return;
            }
        }
        if ($("#fulArchivo_Cortes").val() != "") {
            ext = $('#fulArchivo_Cortes').val().split('.').pop().toLowerCase();
            if ($.inArray(ext, ['', 'xlsx', 'xls']) == -1) {
                alerta('Debe seleccionar archivos de tipo Excel');
                return;
            }
        }
        if ($("#fulArchivo_Adendum").val() != "") {
            ext = $('#fulArchivo_Adendum').val().split('.').pop().toLowerCase();
            if ($.inArray(ext, ['', 'xlsx', 'xls']) == -1) {
                alerta('Debe seleccionar archivos de tipo Excel');
                return;
            }
                ...
                if ($("#ddlPlantilla").val() == '-1') {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            ...
                            //ECONDEÑA  12/11/2015
                            if (result.d.PlantillaDetalles[0].inTipPla == '2' && result.d.PlantillaDetalles[0].idTipPlanRes == '2') {
                                $("#tbCta").show();
                                $("#hdfbtTipoPla").val(1);
