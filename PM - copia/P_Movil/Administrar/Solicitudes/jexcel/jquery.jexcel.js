...
(function ($) {
    ...
    var methods = {
        ...
        createTable: function () {
            ...
            if (options.toolbar && options.toolbar.length) {
                ...
                $.each(options.toolbar, function (k, v) {
                    if (v.type == 'i_fa') {
                        ...
                        $(toolbar).prop('class', 'jexcel-toolbar-item ' + v.content);
                        $(toolbar).prop('style', 'font-size: 18px;padding-top:6px;padding-left:7px;');
                        ...
                        var toolbar = document.createElement('i');
                        if (v.content && v.content == 'separador') {
                            $(toolbar).prop('class', 'material-icons');
                            $(toolbar).prop('style', 'color: #CCCEDB; margin-top: 3px;');
                            $(toolbar).html("more_vert");
                        }
                        else {
                        ...
                        //$(toolbar).prop('class', 'jexcel-toolbar-item');
                        $(toolbar).prop('type', 'text');
                        //if (v.style != null && v.style != "") {
                        //    $(toolbar).prop('style', v.style);
                        //}
                        if (v.maxlength != null && v.maxlength != "") {
                            $(toolbar).prop('maxlength', v.maxlength);
                        }

                        if (v.width != null && v.width != "") {
                            $(toolbar).prop('width', v.width);
                        }
                        if (v.content != null && v.content != "") {
                            $(toolbar).prop('id', v.content);
                        }
                        ...
                        if (v.defaultValue != null && v.defaultValue != "") {
                            $(toolbar).val(v.defaultValue)
                        }
                        if (v.style != null && v.style != "") {
                            $(toolbar).css(JSON.parse(v.style))
                        }
            ...
            ////var tableHeaderAndContentContainer = document.createElement('div');
            ////$(tableHeaderAndContentContainer).prop('class', 'jexcel');
            ////$(tableHeaderAndContentContainer).css({ "max-width": "100%", "width": "100%", "max-height": "100%", "height": "100%", "overflow": "scroll" });
            ...
            ////$(tableHeaderAndContentContainer).append(tableHeaderContainer);
            ////$(tableHeaderAndContentContainer).append(tableContentContainer);
            ////$(this).append(tableHeaderAndContentContainer);
            ...
            if (!$('.jexcel_corner').length) {
                ...
                $.fn.jexcel.pasteControls = function (e) {
                    ...
                    try {
                        if ($($(this)[0].activeElement).attr("id") == "txtCantidadNuevo") {
                            return;
                        }
                        if ($($(this)[0].activeElement).attr("id") == "txtBusqueda") {
                            return;
                        }
                    } catch (e) {
                    }
        ...
        setData: function (data, ignoreSpare) {
            ...
            posColEstado = $.fn.jexcel.defaults[id].colHeaders.indexOf("Estado");
            posColTipoSolicitud = $.fn.jexcel.defaults[id].colHeaders.indexOf("TipoSolicitud");
            posColLinea = $.fn.jexcel.defaults[id].colHeaders.indexOf("Linea");
            posColCodEmpleado = $.fn.jexcel.defaults[id].colHeaders.indexOf("CodEmpleado");
            posColIMEIActual = $.fn.jexcel.defaults[id].colHeaders.indexOf("IMEIActual");
            posColPaquete = $.fn.jexcel.defaults[id].colHeaders.indexOf("Paquete");
            posColDetallePaquete = $.fn.jexcel.defaults[id].colHeaders.indexOf("DetallePaquete");
            ...
            for (j = 0; j < $.fn.jexcel.defaults[id].data.length; j++) {
                ...
                for (i = 0; i < $.fn.jexcel.defaults[id].colHeaders.length; i++) {
                    ...
                    if (i == posColTipoSolicitud) {
                        TipoSolicitud = $.fn.jexcel.defaults[id].data[j][i];
                    }
                    if (i == posColEstado) {
                        Estado = $.fn.jexcel.defaults[id].data[j][i];
                    }
                    if (Estado == "Culminada" || Estado == "Anulada") {
                        $(td).addClass("readonly", "readonly");
                    }
                    else {
                        switch (TipoSolicitud) {
                            case 'Nuevo':
                                if (i == posColLinea) $(td).addClass("readonly", "readonly");
                                if (i == posColIMEIActual) $(td).addClass("readonly", "readonly");
                                if (i == posColPaquete) $(td).addClass("readonly", "readonly");
                                if (i == posColDetallePaquete) $(td).addClass("readonly", "readonly");
                                break;
                            default:
                                break;
                        }
            ...
            if ($(cell).hasClass('readonly') == true) {
                ...
                if (options.columns[position[0]].editor) {
                    ...
                    if (options.columns[position[0]].type == 'checkbox' || options.columns[position[0]].type == 'radio' || options.columns[position[0]].type == 'hidden') {
                        ...
                        if ($.fn.jexcel.defaults[id].rowscolums[position[0]]) {
                            if ($.fn.jexcel.defaults[id].rowscolums[position[0]][position[1]]) {
                                var source = options.rowscolums[position[0]][position[1]].source;
                            }
                        }

                        var pos = $(cell).attr("id").split('-');
                        if ($.fn.jdropdown) {
                            // Create dropdown
                    ...
                    if (options.columns[position[0]].type == 'checkbox' || options.columns[position[0]].type == 'radio') {
                        ...
                        if (value) {
                            ...
                            if ($.fn.jexcel.defaults[id].rowscolums[position[0]]) {
                                if ($.fn.jexcel.defaults[id].rowscolums[position[0]][position[1]]) {
                                    var source = options.rowscolums[position[0]][position[1]].source;
                                    if (typeof options.columns[position[0]].source !== "undefined" && options.columns[position[0]].source != null) {
                                        if (typeof options.rowscolums[position[0]][position[1]].source === "undefined" || options.rowscolums[position[0]][position[1]].source == null) {
                                            var source = options.columns[position[0]].source;
                                        }
                                    }
                                }
            ...
            if (options.allowInsertRow == true) {
                ...
                var createRow = function () {
                    ...
                    for (var col = 0; col < $.fn.jexcel.defaults[id].colHeaders.length; col++) {
                        // New Data
                        if (col == 0) { //Id
                            $.fn.jexcel.defaults[id].data[row][col] = data[col] ? data[col] : nuevoGuid;
                        }
                        else if (col == 1) { //Fecha
                            $.fn.jexcel.defaults[id].data[row][col] = data[col] ? data[col] : fechaActual;
                        }
                        else if (col == 2) { //Estado
                            $.fn.jexcel.defaults[id].data[row][col] = data[col] ? data[col] : estadoDefault;
                        }
                        else {
                ...
                $(this).jexcel('setReadOnly', "0", "2", true); //Estado no editable.
        ...
        setSourceRowColumn: function (row, column, source) {
            var id = $(this).prop('id');
            if (!$.fn.jexcel.defaults[id].rowscolums[column]) {
                $.fn.jexcel.defaults[id].rowscolums[column] = [];
            }
            if (!$.fn.jexcel.defaults[id].rowscolums[column][row]) {
                $.fn.jexcel.defaults[id].rowscolums[column][row] = {};
            }
            $.fn.jexcel.defaults[id].rowscolums[column][row].source = source;
        },
        setReadOnly: function (row, column, readOnly) {
            ...
            ////    $.fn.jexcel.defaults[id].rowscolums[column][row] = {};
            ////}
            ////$.fn.jexcel.defaults[id].rowscolums[column][row].readonly = readOnly;

            var idCell = "#" + column + '-' + row;
            if (readOnly) {
                $(idCell).addClass("readonly", "readonly");
            }
            else {
                $(idCell).removeClass("readonly");
            }
        },
        ...
        setHistory: function (changes, action) {
            if ($.fn.jexcel.ignoreHistory != true) {
                ...
                if (action) {
                    ...
                    if (action.type && action.type == "insertRow") {
                        for (var i = 0; i < changes.length; i++) {
                            changes[i].id = $("#0-" + changes[i].row).text();
                        }
                        history[index].cellChanges = changes;
                    }
                } else {
                    // Create history slot
                    for (var i = 0; i < changes.length; i++) {
                ...
                for (j = 0; j < y; j++) {
                    for (i = 0; i < x; i++) {
                        ...
                        if (v) {
                            ...
                            // Value
                            //data[k] = v;
                            if (v.indexOf("display: none") < 0 && $.trim(v) != "") {
                                data.push('{"' + k + '":"' + v + '"}');
                            }
                        }
                    }
        ...
        contextMenu: function (type, number, e) {
            ...
            if (type == 'col') {
                var existemenuCol = false;
                if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowInsertColumn == true) {
                    existemenuCol = true;
                    contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('insertColumn', 1, null, " + number + ")\">Insertar nueva columna<span></span></a>";
                }
                ...
                if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowChangeNameColumn == true) {
                    existemenuCol = true;
                    contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('setHeader', " + number + ")\">Cambiar de nombre columna</a>";
                }
                if (existemenuCol) {
                ...
                if ($.fn.jexcel.defaults[$.fn.jexcel.current].columnSorting == true) {
                    contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('orderBy', " + number + ", 0)\">Ordenar ascendentemente <span></span></a>";
                    contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('orderBy', " + number + ", 1)\">Ordenar descendentemente <span></span></a>";
                }
                contextMenuContent += "<hr><a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('copy', true)\">Copiar...<span>Ctrl + C</span></a>";
                contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('download')\">Exportar...<span>Ctrl + S</span></a>";
                ...
                if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowInsertRow == true) {
                    contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('insertRow', 1, " + number + ")\">Agregar nueva fila<span></span></a>";
                }
                if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowDeleteRow == true) {
                    contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('deleteRow')\">Eliminar fila(s)<span></span></a>";
                }
                contextMenuContent += "<hr>";
                if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowComments == true) {
                    if (!$($.fn.jexcel.selectedCell).prop('title')) {
                        contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('setComments', $.fn.jexcel.selectedCell, prompt('Comments', ''));\">Agregar comentario<span></span></a><hr>";
                    } else {
                        contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('setComments', $.fn.jexcel.selectedCell, prompt('Comments', $($.fn.jexcel.selectedCell).prop('title')));\">Editar commentario<span></span></a>";
                        contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('setComments', $.fn.jexcel.selectedCell, '');\">Quitar comentario<span></span></a><hr>";
                    }
                }
