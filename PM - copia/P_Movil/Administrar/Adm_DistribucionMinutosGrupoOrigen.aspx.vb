...
Partial Class P_Movil_Administrar_Adm_DistribucionMinutosGrupoOrigen
   ...
   ''<WebMethod()> _
   ''Public Shared Function ListarColModelGrupoOrigen(ByVal vcCodCue As String, ByVal vcValIli As String) As List(Of Object)
   ''    Try
   ...
   ''     Dim dt As DataTable = GrupoOrigen.ListarDistribucionServiciosColumnas(vcCodCue.Replace("&#39", "''"))
   ''     GrupoOrigen.Dispose()
   ...
   ''        colmodel.Add(JQGrid.Columna(dt.Rows(0).Item(1), dt.Rows(1).Item(1), False, True, 200, False, True, ""))
   ...
   ''Public Shared Function ListarGrupoOrigen(ByVal inPagTam As Integer, ByVal inPagAct As Integer, ByVal vcValIli As String, ByVal vcCodCue As String, ByVal inTipBus As String, ByVal vcBus As String) As Object
   ''    Try
   ''        Dim GrupoOrigen As BL_GEN_GrupoOrigen = new BL_GEN_GrupoOrigen(CType(HttpContext.Current.Session("Usuario"), ENT_SEG_Usuario).IdCliente)
   ''        Dim dict As New Dictionary(Of String, Object)

   ''     Dim dt As DataTable = GrupoOrigen.ListarDistribucionServicios(vcValIli, vcCodCue.Replace("&#39", "''"), Convert.ToUInt32(inTipBus), vcBus)
   ''     GrupoOrigen.Dispose()
   ...
   ''        Dim GrupoOrigen As BL_GEN_GrupoOrigen = new BL_GEN_GrupoOrigen(CType(HttpContext.Current.Session("Usuario"), ENT_SEG_Usuario).IdCliente)
   ''        Dim _return As Dictionary(Of String, Object) = GrupoOrigen.GuardarServicios_tmp(vcCodCue, oRegistros, ClaseDistribucion, vcValIli)
   ''     GrupoOrigen.Dispose()
   ''     Return _return
   ''    Catch ex As Exception
   ''        Dim util As New Utilitarios
   ...
   ''        HttpContext.Current.Session("ParametrosGrupoOrigen") = dict
   ''    Catch ex As Exception
   ''        Dim util As New Utilitarios
   ''        util.GrabarLog(ex, HttpContext.Current.Server.MapPath("~/"), Utilitario.NombreSistemaMovil, HttpContext.Current.Session("Usuario"))
   ''        Throw New Exception(UtilitarioWeb.MensajeError)
