...
Partial Class P_Movil_Administrar_Adm_GaleriaModDispositivos
   Inherits System.Web.UI.Page
            ...
            Else
                If Not IsPostBack Then
                    ...
                    If codPlan = 0 Then
                        ...
                        lstOperadores = Operador.Listar("-1", "<< Todos >>")
                    End If
    ...
    Public Shared Function PreciosOperador(ByVal inCodModDis As Integer) As String
        ...
        Try
            ...
            For Each dr As DataRow In dsResult.Tables(0).Rows
                ...
                Else
                    resultado += "PreciosOperador['" + inCodModDis.ToString() + "'].Precios['" + dr("p_inCodOpe").ToString() + "'] = [];"
                    resultado += "PreciosOperador['" + inCodModDis.ToString() + "'].Precios['" + dr("p_inCodOpe").ToString() + "'].dePreLis = '" + DevuelveNumeroFormateado_MultiPais(dr("dePreLis").ToString(), oCultura).ToString() + "';"
                    resultado += "PreciosOperador['" + inCodModDis.ToString() + "'].Precios['" + dr("p_inCodOpe").ToString() + "'].dePreEsp = '" + DevuelveNumeroFormateado_MultiPais(dr("dePreEsp").ToString(), oCultura).ToString() + "';"
                    resultado += "PreciosOperador['" + inCodModDis.ToString() + "'].Precios['" + dr("p_inCodOpe").ToString() + "'].deCosEqu = '" + DevuelveNumeroFormateado_MultiPais(dr("deCosEqu").ToString(), oCultura).ToString() + "';"
                End If
    ...
    <WebMethod()>
    Public Shared Function ListarDispositivos(ByVal vcCodEmp As String, ByVal codPlan As Integer,
                                              ByVal tipSol As Integer, ByVal inCodOpe As Integer,
                                              ByVal TipoServicio As Integer) As List(Of Object)
        ...
        Try
            ...
            If (codPlan <> 0) Then 'dispositivos compatibles con el plan del dispositivo a cambiar
                lstModeloDispositivo = ModeloDispositivo.ListarPorGrupo_PorPlan(vcCodEmp, codPlan, inCodOpe, TipoServicio) 'solicitud tipo 1 - cambio
            Else
                lstModeloDispositivo = ModeloDispositivo.ListarPorGrupo(vcCodEmp, inCodOpe, TipoServicio) 'solicitud tipo 2 - nuevo
            End If

            Dim fila As Integer
            fila = 1
            For Each dr As DataRow In lstModeloDispositivo.Rows
                Dim dictionary As New Dictionary(Of String, String)
                Dim rutaImagen As String = String.Empty
                Dim ruta As String = HttpContext.Current.Server.MapPath("~/Images/ModeloDispositivo" + CarpetaDominio + "\")

                If (dr("imIma").ToString <> "") Then

                    Dim barrImg As Byte() = dr("imIma")
                    Dim archivo As String = dr("P_inCod").ToString & ".jpg"
                ...
                Dim NombreModelo As String = dr("vcNom").ToString.Replace("&#39", "'")


                dictionary.Add("fila", fila.ToString)
                dictionary.Add("P_inCod", dr("P_inCod").ToString)
                dictionary.Add("vcNom", NombreModelo)
                dictionary.Add("vcDes", dr("vcDes").ToString)
                dictionary.Add("imIma", rutaImagen)
                dictionary.Add("btVig", dr("btVig").ToString)

                dictionary.Add("inEst", dr("inEst").ToString)
                dictionary.Add("F_inCodTipSer", dr("F_inCodTipSer").ToString)
                dictionary.Add("picTipMod", dr("picTipMod").ToString)
                dictionary.Add("btSopLin", dr("btSopLin").ToString)

                dictionary.Add("vcTipoChip", dr("vcTipoChip").ToString)
                dictionary.Add("btMosEnSol", dr("btMosEnSol").ToString)
                'dictionary.Add("capacitacion", dr("capacitacion").ToString)

                dictionary.Add("P_inCodTipSer", dr("P_inCodTipSer").ToString)
                dictionary.Add("TipoModelo", dr("TipoModelo").ToString)

                dictionary.Add("IdTipoModeloDispositivo", dr("IdTipoModeloDispositivo").ToString)
                dictionary.Add("GrupoDispotivo", dr("GrupoDispotivo").ToString)
                dictionary.Add("CodigoTipoChip", dr("CodigoTipoChip").ToString)
                dictionary.Add("NombreTipoChip", dr("NombreTipoChip").ToString)
                dictionary.Add("Disponibles", dr("Disponibles").ToString)


                dictionary.Add("dePreLis", dr("dePreLis").ToString)
                If oCultura.vcCodCul.ToString() = "es-PE" Then
                    dictionary.Add("dePreEsp", DevuelveNumeroFormateado(dr("dePreLis").ToString(), oCultura))
                Else
                    dictionary.Add("dePreEsp", DevuelveNumeroFormateado_MultiPais(dr("dePreLis").ToString(), oCultura))
                End If

                If inCodOpe <> 0 AndAlso inCodOpe <> -1 Then 'solcitud de cambio y reposicion
                    If oCultura.vcCodCul.ToString() = "es-PE" Then
                        dictionary.Add("costoCambio", DevuelveNumeroFormateado(dr("dePreLis").ToString, oCultura))
                    Else
                        dictionary.Add("costoCambio", DevuelveNumeroFormateado_MultiPais(dr("dePreLis").ToString, oCultura))
                    End If
                Else
                    dictionary.Add("costoCambio", "-1")
                End If


                If (inCodOpe <> 0 AndAlso inCodOpe <> -1) Then 'solcitud de cambio y reposicion
                    'tdDOper.InnerHtml = dr("vcNomOpe").ToString()
                    dictionary.Add("vcNomOpe", dr("vcNomOpe").ToString)
                Else
                    dictionary.Add("vcNomOpe", "")
                End If

                dictionary.Add("costoReferencial", "-1")


                listatotal.Add(dictionary)
                fila = fila + 1
            Next
