...
Partial Class P_Movil_BienvenidaMovil2
    Inherits System.Web.UI.Page

    Protected Sub P_Movil_BienvenidaMovil2_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            ...
            Try
                If Not IsPostBack Then
                    ...
                    'If hdfAdmin.Value = "1" Then 'Es administrador
                    ...
                    'End If
                    'Solicitud.Dispose() //geig prioridad a las alertas

                    'Me.lblCantidadCambio.Text = oSolicitud.inNumTotCam
                    'Me.lblCantidadNuevo.Text = oSolicitud.inNumTotNue
                    'Me.lblCantidadReposicion.Text = oSolicitud.inNumTotRep
                    'Me.lblCantidadReparacion.Text = oSolicitud.inNumTotRepa
                    'Me.lbltotal.Text = (oSolicitud.inNumTotCam + oSolicitud.inNumTotNue + oSolicitud.inNumTotRep + oSolicitud.inNumTotRepa).ToString() + " Solicitudes"

                    ''geig prioridad a las alertas
                    'If oSolicitud.inNumTotSol = 0 Then
                    '   Me.lbltotal.Visible = False
                    '   divSoli.Visible = False
                    'Else
                    '   Me.lbltotal.Visible = True
                    '   Me.lbltotal.Text = "Mis Solicitudes (" & oSolicitud.inNumTotSol.ToString() & ")"
                    'End If
                    ''----------------

                    'Dim blEsResponsable As Boolean = UtilitarioWeb.TipoSolicitud.ListarTipoSolicitudAprobacion()
                    Dim blEsResponsable As Boolean = UtilitarioWeb.TipoSolicitud.EsResponsableAprobacion()
                    If blEsResponsable Then
                        divSoliPorAprobar.Visible = False 'hasta corregirlo
                        lblSolicitudesPorAprobar.Text = "5 Solicitudes 'Por Aprobar'"
                    Else
                        divSoliPorAprobar.Visible = False
                    ...
                    'If Request.QueryString("Pag") IsNot Nothing Then
                    '    hdfTipoOpcion.Value = Request.QueryString("Pag").ToString
                    'Else
                    '    hdfTipoOpcion.Value = "1"
                    'End If

                    If ddlTipoLinea.SelectedValue = "1" Then
                        LblTextoTipoLinea.Text = "(Staff)"
                    ElseIf ddlTipoLinea.SelectedValue = "2" Then
                        LblTextoTipoLinea.Text = "(Familia)"
                    Else
                        LblTextoTipoLinea.Text = "(Ambos)"
                    End If

                    dashboard = New BL_DashBoard(oUsuario.IdCliente)
                    Dim oCultura As ENT_GEN_Cultura = CType(HttpContext.Current.Session("Cultura"), ENT_GEN_Cultura)

                    'Dim redimiento As List(Of Double) = dashboard.obtenerRendimiento(IIf(hdfCodOrganizacion.Value <> "", oUsuario.F_vcCodInt, oUsuario.Empleado.P_vcCod), "V_SUM_SC_" + ddlPeriodo.SelectedItem.Value, IIf(hdfCodOrganizacion.Value <> "", True, False), hdfCodOperador.Value, hdfCodSucursal.Value, "", ddlTipoLinea.SelectedValue)
                    ...
                    'lblObjetivo.Text = hdfTipoMoneda.Value.ToString + " " + redimiento(0).ToString(strForNum)
                    'lblConsumo.Text = hdfTipoMoneda.Value.ToString + " " + redimiento(1).ToString(strForNum)
