function ArmaGraficoPerCapita(codint, empresa) {
    ...
    var options = {
        ...
        title: {
            text: 'TOP TEN PERCAPITA - MISMO NIVEL',
            style: {
    ...
    $("#div_ajaxpercapita").html('<img src="~/../images/loader.gif">Generando Gráfico...');

$.ajax({
    type: "POST",
    dataType: "json",
    data: "{'fecha_consulta':'" + txt_fecha.GetText() + "','longitud':'" + codint.length + "','telefonia':'" + txt_telefonia.GetText() + "'}",
    contentType: "application/json; charset=utf-8",
    url: "resumen_datos.aspx/ObtieneTopTenPercapita",
    success: function (items) {
        $("#div_ajaxpercapita").hide();

        if (items.d == "{}") {
            $("#blanco_grafico_percapita").show();
            $("#grafico_percapita").hide();
        }
        else {
            $("#blanco_grafico_percapita").hide();
            $("#grafico_percapita").show();
        }
    ...
    error: function (XMLHttpRequest, textStatus, errorThrown) {
          MostrarErrorAjax(XMLHttpRequest, "ObtieneTopTenPercapita", errorThrown);
      }
  });

}

function fillChart2(chart) {
    chartTopTenPerCapita = chart;
}