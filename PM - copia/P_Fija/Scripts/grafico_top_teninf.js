function ArmaGraficoTopTenInf(codint, empresa) {
    ...
    plotOptions: {
        bar: {
            ...
            dataLabels: {
                ...
                formatter: function () {
                    //return etiqueta(this.point.name);
  ...
  $("#div_ajaxtopteninf").html('<img src="~/../images/loader.gif">Generando Gráfico...');
...
$.ajax({
    ...
    url: "resumen_datos.aspx/ListadoTopTenInf",
    success: function (items) {
        $("#div_ajaxtopteninf").hide();

        if (items.d == "{}") {
            $("#blanco_grafico_topten_inf").show();
            $("#grafico_topten_inf").hide();
        }
        else {
            $("#blanco_grafico_topten_inf").hide();
            $("#grafico_topten_inf").show();
        }
    ...
    error: function (XMLHttpRequest, textStatus, errorThrown) {
          MostrarErrorAjax(XMLHttpRequest, "ListadoTopTenInf", errorThrown);
      }
  });

}

function fillChart3(chart) {
    chartTopTenInf = chart;
}