function ArmaGraficoHistoricoDDI(codint) {
        ...
        plotOptions: {
            pie: {
                ...
                dataLabels: {
                    ...
                    formatter: function () {
                        return '<b>' + this.point.name + '</b>: ' + Highcharts.numberFormat(this.percentage.toFixed(2), NumDecimales, SimDec, SepMil) + ' %';
                    }
                },
                showInLegend: true
            }
    ...
    $.ajax({
        type: "POST",
        dataType: "json",
        data: "{'fecha_consulta':'" + txt_fecha.GetText() + "','codint':'" + codint + "','tipo':'ddicosto', 'telefonia':'" + txt_telefonia.GetText() + "'}",
        contentType: "application/json; charset=utf-8",
        url: "resumen_datos.aspx/getDatosHistoricos",
        success: function (items) {

            if (items.d == "{}") {
                $("#blanco_grafico_historico_DDI").show();
                $("#grafico_historico_DDI").hide();
            }
            else {
                $("#blanco_grafico_historico_DDI").hide();
                $("#grafico_historico_DDI").show();
            }
