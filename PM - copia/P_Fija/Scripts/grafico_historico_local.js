function ArmaGraficoHistoricoLocal(codint) {
    ...
    $.ajax({
        type: "POST",
        dataType: "json",
        data: "{'fecha_consulta':'" + txt_fecha.GetText() + "','codint':'" + codint + "','tipo':'loccosto', 'telefonia':'" + txt_telefonia.GetText() + "'}",
        contentType: "application/json; charset=utf-8",
        url: "resumen_datos.aspx/getDatosHistoricos",
        success: function (items) {

            if (items.d == "{}") {
                $("#blanco_grafico_historico_local").show();
                $("#grafico_historico_local").hide();
            }
            else {
                $("#blanco_grafico_historico_local").hide();
                $("#grafico_historico_local").show();
            }
