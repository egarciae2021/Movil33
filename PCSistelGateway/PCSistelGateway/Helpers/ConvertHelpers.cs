...
namespace PCSistelGateway.Helpers
{
    public static class ConvertHelpers
    {
        ...
        public static int Sum(this int? val, params int?[] intParam)
        {
            int result = val ?? 0;
            try
            {
        ...
        public static Decimal Avg(this int? val, params int?[] intParam)
        {
            int result = val ?? 0;
            try
            {
                ...
                return Math.Round(result.ToDecimal() / (intParam.Length + 1).ToDecimal(), 2);
            }
            catch (Exception)
            {
            }
        ...
        public static Decimal Avg(this decimal? val, params decimal?[] intParam)
        {
            var result = val ?? 0;
            try
            {
        ...
        public static DateTime AddMonths(this DateTime date, int meses)
        {

            if (date.Day != DateTime.DaysInMonth(date.Year, date.Month))
                return date.AddMonths(meses);
            else
                return date.AddDays(1).AddMonths(meses).AddDays(-1);
